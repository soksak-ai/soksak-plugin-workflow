...
fn build_ledger_mirrors_reconcile_ledger() {
    ...
    .to_string();
    let (ok, out) = run(&["build-ledger", "--chunk", "chunk", "--kind", "item"], &stdin);
    ...
    assert!(ids.contains(&"a") && ids.contains(&"b"), "item id 보존: {ids:?}");
...
fn validate_draft_reports_no_violations_for_a_valid_doc() {
    ...
    assert_eq!(v["violations"].as_array().map(|a| a.len()), Some(0), "유효 doc → 위반 0: {out}");
}
