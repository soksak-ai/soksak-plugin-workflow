...
function fakeDeps(nodes, execOut, staged) {
  ...
  return {
    ...
    getNode: async (id) => {
      calls.get.push(id);
      return { ok: true, data: { node: { ...byId.get(id), body: byId.get(id).body || "" } } };
    },
...
test("registerPromptTemplates — {role:text} → prompt.put → role→hash 맵", async () => {
  const puts = [];
  const putPrompt = async (text) => { puts.push(text); return { ok: true, data: { hash: "h_" + text.length } }; };
...
test("reconcileTick — 정규화 item(promptHash) 은 prompt.resolve 로 조립 후 exec-one", async () => {
  const resolved = [];
  const deps = {
    listNodes: async () => ({ ok: true, data: { nodes: [{ id: "i1", kind: "item", badge: "검수전" }] } }),
    getNode: async () => ({ ok: true, data: { node: { body: JSON.stringify({ promptHash: "H1", vars: { title: "슬롯" }, schema: { x: 1 } }) } } }),
    resolvePrompt: async (hash, vars) => { resolved.push([hash, vars]); return { ok: true, data: { prompt: "완성:" + vars.title } }; },
...
test("reconcileTick — 정규화 item refs 를 prompt.resolve 로 전달(directive 콘텐츠 주소 deref)", async () => {
  ...
  const deps = {
    listNodes: async () => ({ ok: true, data: { nodes: [{ id: "i1", kind: "item", badge: "검수전" }] } }),
    getNode: async () => ({ ok: true, data: { node: { body: JSON.stringify({ promptHash: "H1", vars: { title: "슬롯" }, refs: { directive: "hD" } }) } } }),
    resolvePrompt: async (hash, vars, refs) => { resolved.push([hash, vars, refs]); return { ok: true, data: { prompt: "완성" } }; },
...
test("reconcileTick — item schemaHash 를 getPrompt 로 deref+parse → exec-one schema", async () => {
  ...
  const deps = {
    listNodes: async () => ({ ok: true, data: { nodes: [{ id: "i1", kind: "item", badge: "검수전" }] } }),
    getNode: async () => ({ ok: true, data: { node: { body: JSON.stringify({ promptHash: "H1", vars: {}, schemaHash: "hS" }) } } }),
    resolvePrompt: async () => ({ ok: true, data: { prompt: "완성 프롬프트" } }),
...
test("reconcileTick — 평탄 정규화 item vars 를 노드 필드(title/description)에서 조립(category 없음 — 부모 그룹 없음)", async () => {
  ...
  const deps = {
    ...
    getNode: async () => ({ ok: true, data: { node: { title: "슬롯≠재고", description: "슬롯은 위치", parentId: "chunk", body: JSON.stringify({ promptHash: "H1", refs: { directive: "hD" } }) } } }),
    resolvePrompt: async (hash, vars, refs) => { resolved.push([hash, vars, refs]); return { ok: true, data: { prompt: "완성" } }; },
    execOne: async () => ({ oxf: "o", result: "ok" }),
    editNode: async () => {},
    poke: async () => {},
...
test("reconcileTick — 하위호환: promptHash 없는 body 는 그대로 exec-one(resolve 안 함)", async () => {
  ...
  const deps = {
    listNodes: async () => ({ ok: true, data: { nodes: [{ id: "i1", kind: "item", badge: "검수전" }] } }),
    getNode: async () => ({ ok: true, data: { node: { body: JSON.stringify({ prompt: "옛 완성", schema: { x: 1 } }) } } }),
    resolvePrompt: async () => { resolveCalled = true; return { ok: true, data: { prompt: "X" } }; },
...
test("reconcileTick — 정규화 item 템플릿 미발견 시 안전 실패(ok:false, 노드 미변경)", async () => {
  ...
  const deps = {
    listNodes: async () => ({ ok: true, data: { nodes: [{ id: "i1", kind: "item", badge: "검수전" }] } }),
    getNode: async () => ({ ok: true, data: { node: { body: JSON.stringify({ promptHash: "MISSING", vars: {} }) } } }),
...
function draftDeps() {
  ...
  return {
    ...
    putPrompt: async (value) => {
      ...
      return { ok: true, data: { hash: hashOf.get(key) } };
    },
    addNode: async (params) => {
      calls.add.push(params);
...
test("reconcileTick — generate DraftDoc: validate 통과 → applyDraftDoc 발행 + status=done + poke", async () => {
  ...
  let hn = 0;
  deps.putPrompt = async () => ({ ok: true, data: { hash: "h" + ++hn } });
  const r = await reconcileTick(deps);
  assert.equal(r.ok, true);
  assert.equal(r.stage, true);
...
test("reconcileTick — generate DraftDoc 검증 실패면 발행 거부(ok:false·노드 미변경·backoff)", async () => {
  ...
  deps.putPrompt = async () => ({ ok: true, data: { hash: "h1" } });
  const r = await reconcileTick(deps);
...
test("reconcileTick — classify 원장 밖 id(환각)는 전량 거부: category edit 0 + ok:false(backoff 재시도)", async () => {
  ...
  const r = await reconcileTick(deps);
  assert.equal(r.ok, false);
  assert.match(r.message, /원장 밖 id: ghost/);
...
test("reconcileTick — classify 중복 배정 거부", async () => {
  ...
  const r = await reconcileTick(deps);
  assert.equal(r.ok, false);
  assert.match(r.message, /중복 배정: i0/);
  assert.equal(deps.calls.edit.length, 0);
});
...
test("reconcileTick — classify category 기록 실패(editNode ok:false)는 성공 위장 없이 ok:false", async () => {
  ...
  deps.editNode = async (id, fields) => {
    ...
    return fields.category ? { ok: false, code: "INTERNAL", message: "거부" } : { ok: true };
  };
  const r = await reconcileTick(deps);
  assert.equal(r.ok, false);
  assert.match(r.message, /category 기록 실패\(i0\)/);
});
...
test("buildSpawnCmd — bin 명시면 직접 spawn, 기본은 sidecar: 이름 참조(경로 해석 = 코어 소유)", () => {
  // 플러그인은 identity 홈 규칙을 모른다 — 코어 process_spawn 이 sidecar:{name} 을 해석(A17).
  assert.deepEqual(buildSpawnCmd("/x/bin/wf", ["exec-one"]), { cmd: "/x/bin/wf", args: ["exec-one"] });
  assert.deepEqual(buildSpawnCmd(null, ["exec-one", "--lang", "ko"]), {
    cmd: "sidecar:workflow",
    args: ["exec-one", "--lang", "ko"],
  });
});
...
test("reconcileTick — audit 결과 없음(result null)은 ok:false — 감사 없는 완결 금지", async () => {
  ...
  const r = await reconcileTick(deps);
  assert.equal(r.ok, false);
  assert.match(r.message, /audit 결과 없음/);
...
test("reconcileTick — classify/audit 원장 materialize 실패는 실행 전 거부(ok:false, execStage 미호출)", async () => {
  ...
  const r = await reconcileTick(deps);
  assert.equal(r.ok, false);
  assert.match(r.message, /원장 materialize 실패\(audit\)/);
...
/** issuerize 테스트 하니스 — 인증·research·design·plan 완료 상태의 덩어리(유닛도 badge 검증됨). */
function issuerizeNodes() {
  return [
    { id: "chunk", kind: "chunk", parentId: null, badge: "o", status: "todo", description: "정련 지시 전문" },
...
test("issuerizeTick — 게이트 충족 시 o 유닛별 body task(파일 실코드화) 발행, x 유닛 제외", async () => {
  const deps = issuerizeDeps(issuerizeNodes());
  const r = await issuerizeTick(deps, "chunk");
  assert.equal(r.ok, true);
  assert.equal(r.issued, 1, "o 유닛 1개만 실코드화 대상(x 유닛 제외)");
  const first = deps.calls.add[0];
  assert.equal(first.kind, "task", "실코드화 실행 = stage task(reconcile 이 exec-stage 로)");
  assert.equal(first.parentId, "chunk");
  const body = JSON.parse(first.body);
  assert.equal(body.workflow, "research", "번들 정본 이름 참조");
  assert.equal(body.stage, "body");
  assert.equal(body.args.file_path, "src/deduct.ts", "유닛 category = 파일 경로");
  assert.match(body.args.pseudocode, /PSEUDO/, "유닛 슈도코드 전문 주입");
  assert.equal(body.args.directive, "정련 지시 전문", "directive = 덩어리 description(단일 진실 §1)");
  assert.equal(body.args.chunkRef, "chunk");
});

test("issuerizeTick — 유닛 미검증(검수전) 거부", async () => {
  const nodes = issuerizeNodes();
  nodes[4].badge = "검수전";
  const r = await issuerizeTick(issuerizeDeps(nodes), "chunk");
  assert.equal(r.ok, false);
  assert.match(r.message, /유닛 미검증/);
});
...
test("issuerizeTick — research 미경유(fact 없음)·미검증 fact·plan 미경유 각각 거부", async () => {
  ...
  assert.match(r.message, /research 미경유/);
  const pending = issuerizeNodes();
  pending[2].badge = "검수전";
  r = await issuerizeTick(issuerizeDeps(pending), "chunk");
  assert.match(r.message, /미검증 1건/);
  const noUnits = issuerizeNodes().filter((n) => n.kind !== "plan-unit");
  r = await issuerizeTick(issuerizeDeps(noUnits), "chunk");
  assert.match(r.message, /plan 미경유/);
});

test("issuerizeTick — 전 유닛 커버(검수전 code·진행 task·o code) 시 멱등 거부", async () => {
  const withCode = issuerizeNodes();
  withCode.push({ id: "c0", kind: "code", parentId: "chunk", title: "src/deduct.ts", category: "src/deduct.ts", badge: "검수전" });
  let r = await issuerizeTick(issuerizeDeps(withCode), "chunk");
  assert.equal(r.ok, false);
  assert.match(r.message, /이미 이슈라이즈/);
  const withTask = issuerizeNodes();
  withTask.push({ id: "t0", kind: "task", parentId: "chunk", title: "실코드화: src/deduct.ts", status: "todo", body: JSON.stringify({ workflow: "research", stage: "body", args: { file_path: "src/deduct.ts" } }) });
  const deps = issuerizeDeps(withTask);
  r = await issuerizeTick(deps, "chunk");
  assert.equal(r.ok, false);
  assert.match(r.message, /이미 이슈라이즈/);
  assert.equal(deps.calls.add.length, 0);
});

test("issuerizeTick — f 반려 코드만 있는 유닛은 재작업 재발행(반려 사유 부기)", async () => {
  const nodes = issuerizeNodes();
  nodes.push({ id: "c0", kind: "code", parentId: "chunk", title: "src/deduct.ts", category: "src/deduct.ts", badge: "f",
    result: JSON.stringify({ oxf: "f", reason: "store 계약 위반 — mutate 반환형 오용" }) });
  const deps = issuerizeDeps(nodes);
  const r = await issuerizeTick(deps, "chunk");
  assert.equal(r.ok, true, JSON.stringify(r));
  assert.equal(deps.calls.add.length, 1, "f 파일만 재발행");
  const body = JSON.parse(deps.calls.add[0].body);
  assert.equal(body.args.file_path, "src/deduct.ts");
  assert.ok(body.args.pseudocode.includes("store 계약 위반"), "반려 사유가 재작업 지시로 부기");
});
...
test("reconcileTick — research stage 는 원장 필수 주입(실패 시 거부) + 멱등 가드(fact 마커)", async () => {
  ...
  assert.match(r2.message, /원장 materialize 실패\(research\)/);
...
function gateDeps(nodes, bodies = {}) {
  return {
    listNodes: async () => ({ ok: true, data: { nodes } }),
    getNode: async (id) => ({ ok: true, data: { node: { ...nodes.find((n) => n.id === id), body: bodies[id] || "" } } }),
  };
}
...
test("researchGate — 미존재/미인증/설명없음 각각 거부", async () => {
  assert.match((await researchGate(gateDeps([]), "chunk")).message, /미존재/);
  assert.match((await researchGate(gateDeps([{ id: "chunk", badge: "검수전", description: "d" }]), "chunk")).message, /미인증/);
  assert.match((await researchGate(gateDeps([{ id: "chunk", badge: "o", description: " " }]), "chunk")).message, /비어있음/);
});
