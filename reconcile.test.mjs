...
test("issuerizeTick — 이미 실코드화 진행/완료(code 노드 또는 body task 존재)는 멱등 거부", async () => {
  const withCode = issuerizeNodes();
  withCode.push({ id: "c0", kind: "code", parentId: "chunk", title: "src/deduct.ts", badge: "검수전" });
  let r = await issuerizeTick(issuerizeDeps(withCode), "chunk");
  assert.equal(r.ok, false);
  assert.match(r.message, /이미 이슈라이즈/);
  const withTask = issuerizeNodes();
  withTask.push({ id: "t0", kind: "task", parentId: "chunk", title: "실코드화: src/deduct.ts", status: "todo", body: JSON.stringify({ workflow: "research", stage: "body", args: {} }) });
