#!/usr/bin/env node
// mcp-verify — 검색 기질(substrate) 게이트. provider.rs search_mcp 가 배선하는 MCP 서버가 리서치
// 프롬프트가 요구하는 도구를 실제로 노출하는지 프로브로 검증한다. 규칙: "노출할 것이다" 가정 배선 금지 —
// 이 게이트를 통과한 서버만 배선한다(z.ai 패키지를 검색으로 착각한 오류의 재발 방지 도구).
//
// 사용: node tools/mcp-verify.mjs            # 기본 배선(context7·ddg) 검증, 하나라도 결핍이면 exit 1
//       node tools/mcp-verify.mjs --call     # 도구 호출까지(실데이터 반환 확인, 네트워크 필요)
import { spawn } from "node:child_process";

// 배선 정본과 동기: server 이름 → {pkg, expect: 노출해야 하는 도구, probe?: 실호출 검증}
const SUBSTRATE = [
  ...
  { name: "ddg-search", pkg: "@oevortex/ddg_search",
    expect: ["web-search"],
    probe: { tool: "web-search", args: { query: "Node.js current LTS version" } } },
];

function session(pkg, env, want) {
  return new Promise((resolve) => {
    const p = spawn("npx", ["-y", pkg], { env: { ...process.env, ...env }, stdio: ["pipe", "pipe", "pipe"] });
    let buf = "", err = "", tools = null, callRes = null, done = false, callId = null;
    ...
    p.stdout.on("data", (d) => {
      ...
      for (const l of lines) { if (!l.trim()) continue; let m; try { m = JSON.parse(l); } catch { continue; }
        if (m.id === 1 && m.result) {
          ...
          if (want) { callId = 2; setTimeout(() => send({ jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: want.tool, arguments: want.args } }), 200); }
          else setTimeout(finish, 100);
        }
        if (m.id === callId) {
          callRes = m.result?.content?.map((x) => x.text || "").join(" ").slice(0, 200) || JSON.stringify(m.error || "").slice(0, 200);
