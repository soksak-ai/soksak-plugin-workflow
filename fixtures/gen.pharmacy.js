...
const verifyVars = (item) => ({ category: item.category || '미분류', title: item.title, description: item.description || '' })   // directive 는 vars 아님 — varRefs 로 콘텐츠 주소 참조(청크당 1행, 항목마다 복붙 X). {{directive}} 는 소비 시점 deref.
...
if (STAGE === 'generate') {
  ...
  for (const g of groups) {
    if (g && g.category) {
      ...
      if (!registered) { groupOpts.registerPrompts = { verify: VERIFY_TMPL, directive: DIRECTIVE }; registered = true }   // verify 템플릿(전역 공유) + directive(청크당 1행) 둘 다 콘텐츠 주소 등록
      ...
      for (const it of (g.items || [])) {
        if (it && it.title) {
          ...
          await agent('', { publish: true, kind: 'item', nodeId: iid, parent: gid,
            ...
            promptRole: 'verify', vars: verifyVars({ category: g.category, title: it.title, description: it.description }), varRefs: { directive: 'directive' } })
          itemIds.push(iid)
        }
        ii = ii + 1
      }
  ...
  for (const a of additions) {
    if (a && a.title) {
      await agent('', { publish: true, kind: 'item', nodeId: 'add' + ai, parent: CHUNK_REF,
        ...
        promptRole: 'verify', vars: verifyVars({ category: a.category, title: a.title, description: a.description }), varRefs: { directive: 'directive' },
