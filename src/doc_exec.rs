...
mod tests {
    ...
    fn gen_first_turn_grounds_by_search() {
        ...
        assert!(!gen.contains("cannot search"), "gen 이 검색을 금지한다 — 첫 턴에서 GROUNDING 분기가 죽는다");
        ...
        assert!(!draft_raw.contains("WebSearch"), "draft.doc.json 에 WebSearch 도구명 지목 잔존 — glm 에서 검색분기 죽음");
        assert!(!research_raw.contains("WebSearch"), "research.doc.json 에 WebSearch 도구명 지목 잔존 — 같은 결함");
        assert!(common.contains("search the web"), "GROUNDING 은 행위(search the web)로 검색을 지시해야 provider 중립");
        // 검색으로 grounding 한 항목은 출처(조문/표준/URL)를 항목에 명시해야 감사 가능 — 근거 증발 방지.
        assert!(common.contains("CITE the grounded source"), "GROUNDING 이 검색 출처 인용을 요구하지 않는다 — grounding 이 감사 불가로 증발한다");
    }
    ...
    fn verdicts_and_changes_require_evidence() {
        for raw in [
            ...
            let req: Vec<&str> = d["values"]["VERIFY_SCHEMA"]["required"]
                ...
                .collect();
            assert!(req.contains(&"reason"), "VERIFY_SCHEMA 가 reason 을 강제하지 않는다 — 증거 없는 판정 통과");
        }
        let draft: Json = serde_json::from_str(include_str!("../workflows/draft.doc.json")).unwrap();
        let dr = draft["prompts"]["draft-review"].as_str().expect("draft-review prompt");
        assert!(dr.contains("name the specific item id"), "draft-review reason 이 대상 id 인용을 요구하지 않는다 — 근거가 우연에 의존");
    }
