...
use crate::reconcile::{
    build_add_params, build_ledger, export_tick, issuerize_tick, next_tick, reconcile_tick, research_gate, resolve_directive, submit_tick, Deps, EditResult, Node, ReconcileState, StageOut,
};
...
// auth env 수집(main.rs auth_env 이관) — ANTHROPIC_* + 토큰/OAuth 우선순위. 스폰 env 경계에서만(PS9).
fn auth_env() -> Result<Vec<(String, String)>, String> {
    let all: Vec<(String, String)> = std::env::vars().filter(|(k, _)| k.starts_with("ANTHROPIC_") || k == "CLAUDE_ACCOUNT_NAME" || k == "CLAUDE_CODE_OAUTH_TOKEN").collect();
    ...
    if !has_token && !has_oauth && !is_codex {
        return Err("프로필 인증 토큰 미설정 — ANTHROPIC_AUTH_TOKEN 또는 CLAUDE_CODE_OAUTH_TOKEN".to_string());
    }
    if has_token {
        Ok(all.into_iter().filter(|(k, _)| k.starts_with("ANTHROPIC_") || k == "CLAUDE_ACCOUNT_NAME").collect())
    } else {
        Ok(all)
    }
}

// ── exec in-process(run_exec_one/run_exec_stage_doc 이관) ────────────────────

/// exec-one — {prompt, schema?, model?} 한 노드 실행 → {oxf, result}. main.rs run_exec_one.
...
fn exec_stage_inprocess(body: &str, mode: StageMode) -> Result<Value, String> {
    let input: Value = serde_json::from_str(body).map_err(|e| format!("exec-stage 입력 파싱: {e}"))?;
    let stage = input.get("stage").and_then(|s| s.as_str()).ok_or("stage 필드 필수")?.to_string();
    // doc 결정: workflow(번들 이름) 또는 skeleton(임베드).
    let doc: Value = if let Some(name) = input.get("workflow").and_then(|w| w.as_str()) {
        let path = crate::paths::bundled_workflow_path(name)?;
        let raw = std::fs::read(&path).map_err(|e| format!("read {path}: {e}"))?;
        serde_json::from_slice(&raw).map_err(|e| format!("parse {name}: {e}"))?
    ...
    match mode {
        StageMode::Assemble => {
            ...
            let mut cap_fn = |prompt: &str, schema: Option<&Value>, label: &str| -> Result<Value, String> {
                captured = Some(json!({ "prompt": build_prompt_with_schema(prompt, None, Some(&lang)), "schema": schema, "label": label }));
...
fn shape_stage_output(stage: &str, events: Vec<NodeEvent>, result: Value) -> Result<Value, String> {
    if stage == "generate" {
        ...
        let children: Vec<Value> = events.iter().filter_map(|ev| serde_json::to_value(ev).ok()).collect();
...
fn to_stage_out(v: Value) -> Result<StageOut, String> {
    if let Some(d) = v.get("draftDoc") {
        ...
        let children = v.get("children").and_then(|c| c.as_array()).cloned().unwrap_or_default();
        let result = v.get("result").cloned().unwrap_or(Value::Null);
        Ok(StageOut::Children { children, result })
    }
}

// ── production Deps — kanban/scheduler 는 중개 cmd(PS13), exec 은 in-process ──
struct ProdDeps<'a> {
    emit: &'a Emit,
...
impl Deps for ProdDeps<'_> {
    ...
    fn edit_node(&self, id: &str, fields: Value) -> EditResult {
        ...
        if env.get("ok").and_then(|v| v.as_bool()) == Some(true) {
            ...
            EditResult::err(env.get("message").and_then(|m| m.as_str()).unwrap_or("").to_string())
        }
    }
...
// 초기 DAG 발행(main.js emitAndRelay) — doc emit → NodeEvent → kanban 릴레이. LLM 0.
fn publish_doc(deps: &dyn Deps, doc: &Value, args: &Value, task_ctx: Option<&Value>) -> Result<usize, String> {
    if !doc_exec::is_doc(doc) {
        return Err("workflow-doc@0.0.1 필요(spec 필드)".to_string());
    }
    let mut no_agent = |_p: &str, _s: Option<&Value>, _l: &str| -> Result<Value, String> { Err("발행(--emit)은 agent 를 호출하지 않는다".to_string()) };
...
fn relay_events(deps: &dyn Deps, events: &[NodeEvent], task_ctx: Option<&Value>) -> Result<usize, String> {
    ...
    for ev in events {
        ...
        if let Some(reg) = ev_val.get("register_prompts").or_else(|| ev_val.get("registerPrompts")) {
            for (r, h) in register_prompt_templates(reg, deps) {
                role_to_hash.insert(r, h);
            }
        }
        let parent = ev_val.get("parent").and_then(|v| v.as_str()).map(|p| key_of.get(p).cloned().unwrap_or_else(|| p.to_string()));
        let blocked_by: Vec<String> = ev_val
            ...
            .unwrap_or_default();
        let params = build_add_params(&ev_val, parent.as_deref(), &blocked_by, task_ctx, &role_to_hash);
...
impl WorkflowService {
    fn new() -> Self {
        let plugin = std::env::var("SOKSAK_SERVICE_PLUGIN").unwrap_or_else(|_| "soksak-plugin-workflow".to_string());
        WorkflowService { state: Mutex::new((ReconcileState::default(), Runtime::default())), poke_schedule: format!("svc:{plugin}:reconcile") }
    }
}
...
fn ok_outcome(v: Value) -> Outcome {
    ...
    if ok {
        Outcome { ok: true, code, message, hints: vec![], data: Some(data) }
    } else {
        Outcome { ok: false, code: code.or(Some("INTERNAL".into())), message: message.or(Some("error".into())), hints: vec![], data: Some(data) }
    }
}
    ...
    fn handle(&self, op: &str, params: Value, ctx: &OpCtx, emit: &Emit) -> Outcome {
        ...
        match op {
            ...
            "export" => {
                ...
                match (chunk, dir) {
                    (Some(chunk), Some(dir)) => {
                        ...
                        let deps_dir = DirDeps { inner: &deps, dir: dir.to_string() };
            ...
            "research" => {
                ...
                let directive = gate.get("directive").and_then(|d| d.as_str()).map(String::from);
...
// run 발행 — idea/skeleton → 초기 DAG 발행(LLM 은 skeleton 생성 시에만). main.js run 핸들러 축약.
fn run_publish(deps: &dyn Deps, runtime: &mut Runtime, params: Value) -> Outcome {
    ...
    let doc: Value = if let Some(sk) = skeleton_str {
        serde_json::from_str(sk).map_err(|e| format!("skeleton 파싱: {e}")).unwrap_or(Value::Null)
...
// refs_dir = 사이드카 번들 references/(self-contained). serve 는 정련 실경로만 노출한다.
fn generate_skeleton_inprocess(idea: &str, model: Option<&str>) -> Result<Value, String> {
    let model = model.unwrap_or(DEFAULT_MODEL);
    let env = auth_env()?;
    let lang = Language::parse("ko");
    let refs_dir = crate::paths::sidecar_root()
        .map(|r| format!("{}/references", r.display()))
        .ok_or("generate-skeleton: references 해석 실패(references/draft-skill.md 기준)")?;
    crate::generate_skeleton::generate_doc(idea, model, Some(&lang), &refs_dir, &env, None)
}
...
/// serve 진입점(PS18) — 코어가 `<bin> serve` 로 스폰. 상주 서비스 시작.
