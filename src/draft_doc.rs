    ...
    for ev in events {
        ...
        match kind.as_str() {
            "item" => {
                ...
                // 첫 item 에 register_prompts 로 공유값 등록(gen.js: 첫 항목에만 얹음).
...
pub fn validate(doc: &DraftDoc) -> Result<(), Vec<String>> {
    ...
    let classify_id = doc.tasks.iter().find(|t| t.stage == "classify").map(|t| t.id.as_str());
    ...
    if let Some(classify) = doc.tasks.iter().find(|t| t.stage == "classify") {
        let cb: std::collections::BTreeSet<&str> = classify.blocked_by.iter().map(|s| s.as_str()).collect();
...
mod tests {
    ...
    /// register_prompts 는 첫 item 에 얹는다(gen.js 평탄 발행 계약). None 이면 미등록 item.
    fn item_ev(id: &str, parent: &str, title: &str, desc: &str, origin: &str, badge: &str, schema: Option<Json>, register: Option<Json>) -> NodeEvent {
        NodeEvent::Add {
            id: id.into(),
    ...
    fn good_events() -> Vec<NodeEvent> {
        ...
        vec![
            item_ev("i0", "chunk", "재고 차감", "판매 시 차감", "user", "검수전", Some(schema_json()), Some(register)),
            item_ev("i1", "chunk", "유통기한 경고", "만료 임박 알림", "agent", "검수전", Some(schema_json()), None),
    ...
    fn build_folds_flat_events_into_normalized_doc() {
        ...
        assert_eq!(doc.chunk_ref, "chunk", "chunk_ref = 첫 item 의 parent(덩어리 id)");
    ...
    fn build_falls_back_to_item_inline_schema_when_register_lacks_schema() {
        ...
        let events = vec![
            item_ev("i0", "chunk", "요건", "설명", "user", "검수전", Some(schema_json()), Some(register)),
        ];
    ...
    fn build_requirement_has_only_unique_fields_no_inline_shared() {
        ...
        for k in ["schema", "directive", "template", "prompt", "category", "category_id", "name"] {
            assert!(!obj.contains_key(k), "요건에 공유값·분류 슬롯 {k:?} 있음(정규화·평탄 위반)");
        }
        assert!(obj.contains_key("id") && obj.contains_key("title"), "요건은 고유 필드만(id/title/description/origin/badge)");
    }
    ...
    fn validate_rule5_rejects_wrong_hunt_blocked_by() {
        ...
        doc.tasks.iter_mut().find(|t| t.stage == "hunt").unwrap().blocked_by = vec!["i0".to_string()];
        let errs = validate(&doc).unwrap_err();
    ...
    fn validate_rule5_rejects_wrong_classify_blocked_by() {
        ...
        doc.tasks.iter_mut().find(|t| t.stage == "classify").unwrap().blocked_by =
            vec!["i0".to_string(), "i1".to_string()];
        let errs = validate(&doc).unwrap_err();
        assert!(errs.iter().any(|e| e.contains("[⑤]") && e.contains("classify")), "classify 트리 위반: {errs:?}");
    }
    ...
    fn validate_rule5_rejects_wrong_audit_blocked_by() {
        ...
        doc.tasks.iter_mut().find(|t| t.stage == "audit").unwrap().blocked_by =
            vec!["i0".to_string(), "i1".to_string(), "hunt".to_string()];
        let errs = validate(&doc).unwrap_err();
    ...
    fn draft_doc_round_trips_json() {
        ...
        assert!(!s.contains("categories"), "평탄 DraftDoc 에 categories 필드 없음");
    }
