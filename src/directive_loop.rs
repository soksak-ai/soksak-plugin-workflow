...
fn call_json<T: DeserializeOwned>(prompt: String, tools: Vec<String>, model: &str, cfg: &LoopConfig) -> Result<T, String> {
    ...
    for attempt in 0u64..3 {
        ...
        match run_agent_text(&AgentRequest { prompt: prompt.clone(), model, allowed_tools: tools.clone(), timeout_secs: 3600, system_prompt: None, schema: None, effort: "xhigh".into(), text_only: false }, &cfg.agent_env) {
...
fn exec_verify(directive: &str, batch: &[Topic], all: &[Topic], cfg: &LoopConfig) -> Result<Vec<Verification>, String> {
    let to_verify = batch
        .iter()
...
{{"verifications":[{{"id":"...","status":"{OK}"|"{FAIL}"|"{FATAL}","origin":"user"|"agent"|"search","reason":"...","verified_value":"...","sources":["..."]}}]}}"#,
    ...
    let vr: VerifyResult = call_json(prompt, vec!["WebSearch".into(), "WebFetch".into()], &cfg.verifier_model, cfg)?;
    Ok(vr.verifications)
}
...
Output ONLY the spec markdown."#,
    ...
    run_agent_text(&AgentRequest { prompt, model: &cfg.exec_model, allowed_tools: vec![], timeout_secs: 3600, system_prompt: None, schema: None, effort: "xhigh".into(), text_only: false }, &cfg.agent_env)
}
...
pub fn run_loop(directive: &str, ledger: &mut Ledger, cfg: &LoopConfig) -> Result<Outcome, String> {
    ...
    for round in 1..=cfg.max_rounds {
        ...
        if !blank_ids.is_empty() {
            ...
            let chunks: Vec<Vec<String>> = blank_ids.chunks(chunk_size).map(|c| c.to_vec()).collect();
                ...
                let verifs: Vec<Vec<Verification>> = std::thread::scope(|s| {
                    ...
                    handles
                        ...
                        .map(|h| match h.join() {
                            Ok(Ok(v)) => v,
                            Ok(Err(e)) => {
                                eprintln!("  [batch 검증 실패(공백 유지·재시도): {}]", e.chars().take(100).collect::<String>());
                                vec![]
                            }
                            Err(_) => {
        ...
        if let Some(ft) = ledger.topics.iter().find(|t| t.status == FATAL) {
            ...
            abort_reason = format!(
                "[f] {}: {}",
                ft.id,
                ft.history.last().map(|h| h.reason.clone()).unwrap_or_default()
            );
            break;
        }
        ...
        let additions = exec_hunt(directive, &ledger.topics, cfg).unwrap_or_else(|e| {
            eprintln!("  [누락사냥 실패(이번 라운드 추가 없음): {}]", e.chars().take(100).collect::<String>());
            vec![]
        });
...
mod tests {
    ...
    fn apply_verification_sets_status_and_value() {
        ...
        apply_verifications(
            &mut v,
            &[Verification { id: "a".into(), status: OK.into(), origin: "search".into(), reason: "ok".into(), verified_value: "X".into(), sources: vec!["u".into()] }],
            1,
        );
        assert_eq!(v[0].status, OK);
    ...
    fn correction_bumps_version() {
        ...
        apply_verifications(
            &mut v,
            &[Verification { id: "a".into(), status: OK.into(), origin: "agent".into(), reason: "정정".into(), verified_value: "new".into(), sources: vec![] }],
            2,
        );
    ...
    fn promotions_dedupe() {
        ...
        let n = apply_promotions(&mut v, &[Promote { id: "a".into(), subject: "dup".into(), origin: "agent".into() }, Promote { id: "b".into(), subject: "new".into(), origin: "search".into() }]);
