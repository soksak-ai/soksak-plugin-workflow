...
fn call_json<T: DeserializeOwned>(prompt: String, tools: Vec<String>, model: &str, cfg: &LoopConfig) -> Result<T, String> {
    ...
    for attempt in 0u64..3 {
        ...
        match run_agent_text(&AgentRequest { prompt: prompt.clone(), model, allowed_tools: tools.clone(), timeout_secs: 3600, system_prompt: None, schema: None, effort: "xhigh".into() }, &cfg.agent_env) {
...
Output ONLY the spec markdown."#,
    ...
    run_agent_text(&AgentRequest { prompt, model: &cfg.exec_model, allowed_tools: vec![], timeout_secs: 3600, system_prompt: None, schema: None, effort: "xhigh".into() }, &cfg.agent_env)
}
