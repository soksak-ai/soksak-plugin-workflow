//! DraftDoc 발행 + prompt 템플릿 등록(main.js applyDraftDoc·registerPromptTemplates 이식).
...
pub fn register_prompt_templates(register_prompts: &Value, deps: &dyn Deps) -> Vec<(String, String)> {
    let mut out = Vec::new();
...
/// DraftDoc 배치 발행(main.js applyDraftDoc) — verify_contract 템플릿/directive/schema → 해시,
/// requirements → item 노드(평탄), tasks → task 노드(blockedBy DraftDoc id→칸반 id 해석). 반환=발행 수.
pub fn apply_draft_doc(deps: &dyn Deps, doc: &Value, chunk_kanban_id: Option<&str>, task_ctx: Option<&Value>) -> Result<usize, String> {
    let vc = doc.get("verify_contract").cloned().unwrap_or_else(|| json!({}));
    ...
    if let (Some(cr), Some(ck)) = (doc.get("chunk_ref").and_then(|v| v.as_str()), chunk_kanban_id) {
        key_of.insert(cr.to_string(), ck.to_string());
    }
    let mut published = 0usize;
    let initial_badge = vc.get("initial_badge").and_then(|v| v.as_str()).unwrap_or("검수전");
    ...
    if let Some(reqs) = doc.get("requirements").and_then(|v| v.as_array()) {
        for r in reqs {
            ...
            params.insert("title".into(), r.get("title").cloned().unwrap_or(Value::Null));
            ...
            params.insert("badge".into(), json!(r.get("badge").and_then(|v| v.as_str()).unwrap_or(initial_badge)));
