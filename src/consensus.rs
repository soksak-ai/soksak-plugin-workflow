...
pub fn apply_changes(items: &[Value], changes: &[Value], round: u32) -> ChangeSet {
    ...
    for c in changes {
        ...
        let reason = c.get("reason").and_then(|v| v.as_str()).unwrap_or("").trim().to_string();
        match op {
            "add" => {
            ...
            "remove" | "reraise" => {
                ...
                let (from, to) = if op == "remove" { ("o", "x") } else { ("x", "o") };
                if cur != from {
                    continue; // remove 는 o 만, reraise 는 x 만 — 전이 불일치 방어
                }
                let mut history: Vec<Value> = item.get("history").and_then(|h| h.as_array()).cloned().unwrap_or_default();
                history.push(json!({ "round": round, "action": op, "reason": reason }));
                edits.push(ItemEdit { id: id.to_string(), state: to.to_string(), history });
            }
            _ => {}
        }
    }
    let converged = creates.is_empty() && edits.is_empty();
    ChangeSet { creates, edits, converged }
}
...
mod tests {
    ...
    fn changes_add_creates_item_with_history() {
        let cs = apply_changes(&[], &[json!({ "op": "add", "title": "약 등록면", "description": "제형·용량·용법", "reason": "중심 엔티티 입력면" })], 1);
        assert_eq!(cs.creates.len(), 1);
        let it = &cs.creates[0];
        assert_eq!(it["state"], "o");
        assert_eq!(it["title"], "약 등록면");
        assert_eq!(it["history"][0], json!({ "round": 1, "action": "add", "reason": "중심 엔티티 입력면" }));
    ...
    fn changes_remove_o_to_x_appends_history() {
        ...
        let cs = apply_changes(&items, &[json!({ "op": "remove", "id": "a6", "reason": "a7 통합대장이 흡수 — 중복" })], 2);
        ...
        assert_eq!(cs.edits[0].history[1], json!({ "round": 2, "action": "remove", "reason": "a7 통합대장이 흡수 — 중복" }));
    }

    #[test]
    fn changes_reraise_x_to_o_preserves_full_trail() {
        let items = vec![json!({ "id": "a5", "state": "x", "title": "입고 검수", "history": [
            { "round": 1, "action": "add", "reason": "재고정확도" },
            { "round": 2, "action": "remove", "reason": "i0 에 암시됨" }
        ] })];
        let cs = apply_changes(&items, &[json!({ "op": "reraise", "id": "a5", "reason": "독립 상태전이 — R2 반박" })], 3);
        assert_eq!(cs.edits[0].state, "o", "reraise → o");
        assert_eq!(cs.edits[0].history.len(), 3, "add·remove·reraise 전 이력 보존");
