...
- **C2 CLI 멱등**: next→submit 으로 확정 ≥1, 재제출 ALREADY_DONE. 증거: 응답+보드 전이.
- **C2b export**: 확정 code 노드 전부 파일로, 파일 수=노드 수. 증거: 파일 목록.
- **C3 대회 판정**: 동결 입력 실측→지표 표→원칙 10 심판→기본 방법론 확정 커밋.
...
- D2 유닛만 · D3 진행 중(7턴 중 1턴) · D4 진행 중(순수부 작성, 커맨드·스킬 미완) · D5 완료 · D6 미착수
- C1~C5 전부 미충족 — 완료 아님.
